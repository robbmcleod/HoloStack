...
%
% The display image can, optionally, be imported into the display field 
% with a true/false tag after the filename:
%
%   dm3struct = DM3Import( 'filename.dm3', true );
% 
...
function [dm3struct] = DM3Import( dm3filename, importdisplay )
    global dm3file;
    global endianstyle;
    
    if( ~exist( 'importdisplay') || isempty( importdisplay ) )
        importdisplay = false;
    end
    ...
    for index = 1:numel(datatypeloc)
        ...
        if( datatype(index) == 23 ) % Display (throw away)
            if( importdisplay )
                ...
                % matrix.
                % Does reshape change the cast?
                rgbdisplay = readTagData( dataloc(index) );
                
                % Data is stored as 32-bit int
                rgbdisplay = typecast( rgbdisplay, 'uint8' );
                % Display is stored as 'Blue, Green, Red, Transparency'
                % We need to store it as Red-Green-Blue and delete the
                % transparency
                rgbdisplay = reshape( rgbdisplay, 4, xdim, ydim );
                
                % Re-arrange dimensions
                rgbdisplay = permute( rgbdisplay, [3 2 1] );
                % Delete the transparency byte and must also re-arrange from BGR to RGB
                rgddisplay = rgbdisplay(:,:,[3,2,1]);

                % Create and population a [xdim ydim 3] matrix for the
                % display
                dm3struct.display{displayindex} = rgddisplay;
            else
        ...
        elseif( datatype(index) == 7 || datatype(index) == 10 || datatype(index) == 11 || datatype(index) == 1 ) % 2-D image
            ...
            % Test of 3-d stack
            zdimloc = regexp( metadm3file(ydimloc+4:N), '%%%%' );
            if( ~isempty( zdimloc ) );
                zdimloc = zdimloc(1) + (ydimloc+4) - 1;
                zdim = readTagData( zdimloc );
            else
            ...
            % Since data is in a simple vector format, we need to reshape it into a
            % matrix.
            if( isempty(zdim) ) % 2-D image
                dm3struct.image_data = reshape( readTagData( dataloc(index) ), xdim, ydim );
            else % 3-D image stack
                dm3struct.image_data = reshape( readTagData( dataloc(index) ), xdim, ydim, zdim );  
            end
%             dm3struct.image_data{imageindex} = reshape( readTagData( dataloc(index) ), xdim, ydim );
    ...
    % Clean up the data, correct for image tranposition, typecast most data
    % to double, etc.
    if( isfield( dm3struct, 'image_data' ) )
        % Transpose matrix to get x and y-axis same as Digital
        % Micrograph
        % This shouldn't break on 2-D data.
        dm3struct.image_data = permute(dm3struct.image_data,[2 1 3]);
        % Convert to double
        dm3struct.image_data = cast( dm3struct.image_data, 'double' );
    end
    
    if( isfield( dm3struct, 'spectra_data' ) )
        % Convert to double
        for I = 1:spectracount
            dm3struct.spectra_data{I} = cast( dm3struct.spectra_data{I}, 'double' );
        end
    end
...
function [td] = readTagData( fileloc )
    ...
    % disp( ['Debug A, tagdatatype ', num2str(tagdatatype)] );
...
function [td] = readDM3Array()
    ...
    if strcmp( tagdatatype, 'DM3struct' )
        ...
        disp( ['DM3Import assumes any array of struct is a complex image, arraylength = ', num2str(arraylength)] )
        
        % It seems for complex images now that the array length is 0, 
        %  disp( 'DEBUG: complex image handling' );
        % arraylength = fread(dm3file, 1, 'uint32' );
        % What is this extra stuff in the complex images?
        % fread(dm3file, 3, 'uint32' );
